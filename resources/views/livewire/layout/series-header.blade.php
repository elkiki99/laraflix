            ...
            return collect(Http::withToken(config('services.tmdb.token'))->get('https://api.themoviedb.org/3/trending/tv/day')->json()['results'])
                ->map(function ($serie) {
                    return [
                        ...
                        'imgAlt' => $serie['name'],
                        'title' => $serie['name'],
                        'description' => Str::limit($serie['overview'], 400, '...') ?? $serie['overview'],
                    ];
                })
                ->shuffle()
                ->random(8)
                    ...
                    <div class="flex items-end justify-start md:min-h-[90vh] min-h-[80vh] mx-auto max-w-7xl">
                        <div class="z-20 p-4 text-white">
                            <div class="space-y-2">
                                ...
                                <div class="pt-2">
                                    <a wire:navigate :href="`{{ route('series.show', '') }}/${slide.id}`">
                                        <x-primary-button class="px-16">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <p class="text-xs font-bold text-black md:text-md">Go to series</p>
                                            </div>
                                        </x-primary-button>
                                    </a>
                                </div>
